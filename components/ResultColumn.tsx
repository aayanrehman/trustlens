...
const GRADE_COLOR: Record<string, string> = { A: "text-ok", B: "text-ok", C: "text-ink", D: "text-accent", F: "text-accent" };
  ...
  return (
    <div className="py-2 border-t rule">
      ...
      <div className="mt-1 space-y-0.5">
        {rows.map(([k, p]) => (
          <div key={k} className="flex items-center gap-2 text-[11px]"><span className="w-40 truncate text-ink-2">{humanize(k)}</span><span className="flex-1 h-1.5 bg-rule/60"><span className="block h-full bg-accent grow" style={{ width: `${Math.round(p * 100)}%` }} /></span><span className="mono w-8 text-right">{Math.round(p * 100)}</span></div>
        ))}
      </div>
    </div>
  );
...
export default function ResultsGrid({ results, thresholds, questions }: { results: SiteResult[]; thresholds: Thresholds; questions: QuestionDef[] }) {
  ...
  rows.push(results.map((r) => (
    <div key={"h" + r.url} className={`${cell} rise`}>
      ...
      {r.client_only_shell && <p className="mt-2 text-xs border border-accent p-2"><span className="eyebrow text-accent">crawler-invisible</span> Ships an empty HTML shell; most AI crawlers see nothing.</p>}
      {r.status !== "ok" && <p className="mt-2 text-sm text-accent">{r.status === "blocked" ? "Skipped: " : "Failed: "}{r.reason}</p>}
    </div>
  )));
