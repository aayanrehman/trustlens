...
export default async function AuditPage({ params, searchParams }: Props) {
  ...
  const labels = ["SEO", "GEO", "Trust", "Discoverability"];
    ...
    <main className="mx-auto max-w-5xl px-6 py-14 grid gap-10 md:grid-cols-2 items-center">
      <div className="rise">
        ...
        <div className="mt-8 flex gap-2 text-sm">
          <Link href={`/?u=${encodeURIComponent(p.h)}`} className="bg-ink text-paper px-4 py-2 hover:bg-accent">Score your site →</Link>
          <a href={`/api/og?d=${d}`} download={`trustlens-${slug}.png`} className="border border-ink px-4 py-2">Download card</a>
        </div>
      </div>
